...
class Kota extends AUTH_Controller {
	...
	public function index() {
		...
		$data['deskripsi'] 	= "Manage Data Kelurahan";
		$data['dataKecamatan'] = $this->M_kota->select_kecamatan();
	...
	public function prosesUpdate() {
		$this->form_validation->set_rules('id_kelurahan', 'id_kelurahan', 'trim|required');
		...
		if ($this->form_validation->run() == TRUE) {
			...
			if ($result > 0) {
				$out['status'] = '';
				$out['msg'] = show_succ_msg('Data Kel / Desa Berhasil diupdate', '20px');
			} else {
				$out['status'] = '';
				$out['msg'] = show_succ_msg('Data KEl / Desa Gagal diupdate', '20px');
			}
		} else {
			$out['status'] = 'form';
			$out['msg'] = show_err_msg(validation_errors());
