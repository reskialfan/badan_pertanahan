		...
		if ($this->form_validation->run() == TRUE) {
			if (md5($this->input->post('passLama')) == $this->userdata->password) {
				if ($this->input->post('passBaru') != $this->input->post('passKonf')) {
					...
					$this->session->set_flashdata('error_msg', 'Password Baru dan Konfirmasi Password harus sama.');
					redirect('Profile');
				} else {
					$data = [
					...
					if ($result > 0) {
						...
						$this->session->set_flashdata('error_msg', 'Password Berhasil diubah.');

						redirect('Profile');
					} else {
						//$this->session->set_flashdata('msg', show_err_msg('Password Gagal diubah'));
						$this->session->set_flashdata('error_msg', 'Password Gagal diubah.');
						redirect('Profile');
					}
				}
			} else {
